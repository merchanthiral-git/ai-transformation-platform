      ...
      <div className="px-6 py-6 space-y-5">
        ...
        {!loading && narrative && <>
          {editing ? (
            ...
            <div onClick={() => setEditing(true)} className="cursor-text rounded-xl p-5" style={{ borderLeft: "2px solid rgba(244,168,58,0.25)", background: "var(--surface-1)" }} title="Click to edit">
              {narrative.split("\n\n").map((para, i) => {
          ...
          <div className="flex items-center gap-2 pt-4" style={{ borderTop: "1px solid var(--border)" }}>
            <button onClick={generateNarrative} disabled={loading} className="px-4 py-2 rounded-xl text-[14px] font-semibold text-white disabled:opacity-50 glow-pulse" style={{ background: "linear-gradient(135deg, var(--accent-primary), var(--teal))" }}>
...
export function ImpactSimulator({ onBack, onNavigate, model, f, jobStates, simState, setSimState, viewCtx }: { onBack: () => void; onNavigate?: (id: string) => void; model: string; f: Filters; jobStates: Record<string, JobDesignState>; simState: { scenario: string; custom: boolean; custAdopt: number; custTimeline: number; investment: number }; setSimState: (s: { scenario: string; custom: boolean; custAdopt: number; custTimeline: number; investment: number }) => void; viewCtx?: ViewContext }) {
  ...
  const readinessData = useMemo(() => {
    const br = backendReadiness as unknown as ReadinessResponse | null;
    if (br && br.dimensions && Object.keys(br.dimensions).length > 0) {
      // Backend has real readiness data
      return { source: "uploaded" as const, data: br };
    }
    return { source: "demo" as const, data: null };
  }, [backendReadiness]);
  ...
  return <div>
    ...
    {realJobs ? <div className="bg-[rgba(139,168,122,0.08)] border border-[var(--success)]/30 rounded-lg px-4 py-2 mb-4 text-[15px] text-[var(--success)]">✓ Using your Work Design data — {realJobs.length} roles from your submitted jobs</div> : <div className="bg-[rgba(244,168,58,0.08)] border border-[var(--warning)]/30 rounded-lg px-4 py-2 mb-4 text-[15px] text-[var(--warning)]">Using demo data — complete jobs in the Work Design Lab to see your real numbers here</div>}
    ...
    {tab === "scenarios" && <div>
      ...
      {custom && <div className="bg-[var(--surface-1)] border border-[var(--accent-primary)] rounded-xl p-5 mb-4" style={{ boxShadow: "0 0 0 1px rgba(244,168,58,0.15)" }}>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            style={{ background: "linear-gradient(135deg, var(--accent-primary), var(--teal))", boxShadow: "0 2px 8px rgba(244,168,58,0.25)" }}
          >
      ...
      {scenSub === "waterfall" && <Card title="Capacity Waterfall">
        ...
        <div className="flex items-end gap-1 justify-center mb-4" style={{ height: 280 }}>
          {[{ label: "Current", value: totals.cur, color: "var(--accent-primary)", detail: `${activeJobs.length} roles, ${totals.cur.toLocaleString()}h/mo total capacity` },
            { label: "AI Removed", value: -totals.rel, color: "var(--risk)", detail: `${totals.rel.toLocaleString()}h freed by automation (${totalPct}% of current)` },
            { label: "Redesigned", value: Math.round(totals.rel * 0.15), color: "var(--success)", detail: `~${Math.round(totals.rel * 0.15).toLocaleString()}h added through role redesign and new tasks` },
            { label: "Net Future", value: totals.fut, color: "var(--surface-2)", detail: `${totals.fut.toLocaleString()}h/mo future capacity (${Math.round(totals.fut / Math.max(totals.cur, 1) * 100)}% of current)` },
          ].map((seg, i) => {
          ...
          <ExpandableChart title="Capacity Waterfall">
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={deptData} layout="vertical">
                ...
                <Tooltip contentStyle={{ background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 15, color: "var(--text-primary)" }} />
                <Legend wrapperStyle={{ fontSize: 15 }} />
                <Bar dataKey="reduced" name="Reduced" fill="var(--risk)" stackId="a" />
                <Bar dataKey="redeployed" name="Redeployed" fill="var(--accent-primary)" stackId="a" />
                <Bar dataKey="created" name="Created" fill="var(--success)" stackId="a" />
              </BarChart>
            </ResponsiveContainer>
          </ExpandableChart>
          </>;
          ...
          <ExpandableChart title="Scenario Radar">
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={savedScenarios.map((s, i) => ({ name: s.name, savings: s.totals.savings, fill: COLORS[i % COLORS.length] }))}>
                  ...
                  <Tooltip contentStyle={{ background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 15, color: "var(--text-primary)" }} formatter={(v: number) => `$${v.toLocaleString()}`} />
    ...
    {tab === "mgrready" && <div className="space-y-5">
      <Card title="Manager Impact Timeline — HRBP Weekly Action List">
        ...
        {(() => {
          ...
          return <div className="space-y-2">{mgrActions.sort((a, b) => a.month - b.month).map(ma => <div key={ma.id} className="flex items-center gap-3 rounded-xl p-4" style={{ background: ma.done ? "rgba(139,168,122,0.04)" : "var(--surface-1)", border: `1px solid ${ma.done ? "var(--success)" : "var(--border)"}`, borderLeft: `4px solid ${catColors[ma.category] || "var(--text-muted)"}`, opacity: ma.done ? 0.6 : 1 }}>
        ...
        {(() => {
          ...
          return <div>
            ...
            {sel && <div className="space-y-4">
              {/* Direct impact */}
              <div className="rounded-xl p-4 border-l-4" style={{ borderLeftColor: "var(--accent-primary)", background: "rgba(244,168,58,0.04)" }}>
              ...
              {/* Conflicts */}
              {sel.conflicts.length > 0 && <div className="rounded-xl p-4 border border-[var(--risk)]/20 bg-[rgba(232,122,93,0.04)]">
          ...
          return <div className="space-y-3">{teamData.map(t => {
            ...
            return <div key={t.mgr} className="rounded-xl border border-[var(--border)] bg-[var(--surface-1)] p-4" style={{ boxShadow: "var(--shadow-1)" }}>
              <div className="flex items-center justify-between mb-2">
                ...
                {t.issues > 0 && <span className="px-2 py-0.5 rounded-full text-[12px] font-bold bg-[rgba(232,122,93,0.1)] text-[var(--risk)]">{t.issues} issue{t.issues > 1 ? "s" : ""}</span>}
              </div>
              {/* Single progress bar with milestone markers */}
              <div className="relative h-3 bg-[var(--surface-3)] rounded-full overflow-hidden">
                <div className="absolute inset-y-0 left-0 rounded-full transition-all" style={{ width: `${pct}%`, background: pct >= 75 ? "var(--success)" : pct >= 25 ? "var(--warning)" : "var(--risk)" }} />
...
function NegotiateTab({ projectId, model, savedScenarios, setSavedScenarios }: { projectId: string; model: string; savedScenarios: SavedScen[]; setSavedScenarios: (s: SavedScen[]) => void }) {
  ...
  const statusBg = (s: string) => s === "met" ? "rgba(139,168,122,0.1)" : s === "partial" ? "rgba(244,168,58,0.1)" : "rgba(232,122,93,0.1)";
  const feasColor = (f: string) => f === "fully_achievable" ? "var(--success)" : f === "partially_achievable" ? "var(--warning)" : "var(--risk)";
  const feasBg = (f: string) => f === "fully_achievable" ? "rgba(139,168,122,0.08)" : f === "partially_achievable" ? "rgba(244,168,58,0.08)" : "rgba(232,122,93,0.08)";
  ...
  return <div className="space-y-5">
    ...
    <Card title="What do you need this transformation to achieve?">
      ...
      <div className="space-y-2 mb-4">
        {constraints.map(c => <div key={c.id} className="flex items-center gap-2 p-2 rounded-lg bg-[var(--surface-2)] border border-[var(--border)]">
          ...
          <button onClick={() => updateConstraint(c.id, "priority", c.priority === "must_have" ? "nice_to_have" : "must_have")} className="px-2.5 py-1.5 rounded-lg text-[11px] font-bold transition-all shrink-0" style={{ background: c.priority === "must_have" ? "rgba(232,122,93,0.1)" : "rgba(167,139,184,0.1)", color: c.priority === "must_have" ? "var(--risk)" : "var(--purple)", border: `1px solid ${c.priority === "must_have" ? "rgba(232,122,93,0.2)" : "rgba(167,139,184,0.2)"}` }}>{c.priority === "must_have" ? "Must Have" : "Nice to Have"}</button>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1" />
        <button onClick={negotiate} disabled={loading || constraints.length === 0} className="px-6 py-2.5 rounded-xl text-[14px] font-bold text-white transition-all hover:scale-[1.02] disabled:opacity-40" style={{ background: "linear-gradient(135deg, var(--accent-primary), var(--teal))" }}>
    ...
    {loading && <><div className="rounded-xl border border-[var(--border)] bg-[var(--surface-1)] p-6">
      ...
      <div className="h-1.5 bg-[var(--surface-3)] rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-300" style={{ width: `${progress}%`, background: "linear-gradient(90deg, var(--accent-primary), var(--warning))" }} /></div>
    ...
    {result && !loading && !result.error && <>
      ...
      {result.tradeoffs.length > 0 && <Card title="Tradeoffs">
        <div className="space-y-3">{result.tradeoffs.map((t, i) => <div key={i} className="rounded-xl p-4 bg-[rgba(244,168,58,0.06)] border border-[var(--warning)]/20">
          ...
          <div className="space-y-1">{t.resolution_options.map((opt, oi) => <div key={oi} className="text-[13px] text-[var(--text-secondary)] pl-3 border-l-2 border-[var(--warning)]/30 py-0.5">{opt}</div>)}</div>
        </div>)}</div>
      </Card>}
...
function StressTestTab({ projectId, model }: { projectId: string; model: string }) {
  ...
  return <div className="space-y-5">
    ...
    {loading && <><div className="rounded-xl border border-[var(--border)] bg-[var(--surface-1)] p-6">
      ...
      <div className="h-1.5 bg-[var(--surface-3)] rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-300" style={{ width: `${progress}%`, background: "linear-gradient(90deg, var(--risk), var(--warning))" }} /></div>
    ...
    {result && !loading && !result.error && <>
          ...
          <div className="space-y-2.5">{result.vulnerability_map.map((vm, i) => {
            ...
            return <div key={i}>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[11px] font-data" style={{ color: drop > 3 ? "var(--risk)" : drop > 1 ? "var(--warning)" : "var(--success)" }}>{vm.current_resilience} → {vm.post_shock_resilience}</span>
              </div>
              <div className="flex gap-1 h-3">
                <div className="flex-1 bg-[var(--surface-3)] rounded-full overflow-hidden relative">
                  ...
                  <div className="absolute inset-y-0 left-0 rounded-full" style={{ width: `${vm.post_shock_resilience * 10}%`, background: vm.post_shock_resilience < 4 ? "var(--risk)" : vm.post_shock_resilience < 7 ? "var(--warning)" : "var(--success)" }} />
                </div>
              </div>
      ...
      {prevResult && <div className="rounded-xl p-3 bg-[rgba(167,139,184,0.06)] border border-[var(--purple)]/20 flex items-center gap-3">
