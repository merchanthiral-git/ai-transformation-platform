...
export const COLORS = ["var(--amber)","var(--dusk)","var(--sage)","var(--coral)","var(--amber)","var(--sky-gold)","var(--dusk)","#2dd4bf"];

/** Track colors — canonical source of truth (Vista palette) */
export const TRACK_COLORS: Record<string, string> = {
  S: "var(--amber)",  // Support — amber
  P: "var(--sky-gold)",  // Professional — gold
  M: "var(--dusk)",  // Management — dusk
  E: "var(--coral)",  // Executive — coral
  T: "var(--dusk)",  // Technical — dusk
};

/** Severity scale — Vista semantic */
export const SEVERITY_COLORS = {
  critical: "var(--coral)",  // rose
  warning: "var(--amber)",   // amber
  info: "var(--amber)",      // cyan
  success: "var(--sage)",   // emerald
} as const;
...
export const PHASES = [
  { id: "discover", label: "Discover", icon: "🔍", color: "var(--amber)", desc: "Understand where you are", guidance: "Start by understanding your organization's workforce structure, job architecture, and AI readiness.", modules: ["dashboard", "snapshot", "skillshift", "jobarch"] },
  { id: "diagnose", label: "Diagnose", icon: "🩺", color: "var(--amber)", desc: "Find what matters most", guidance: "Now that you understand the landscape, let's identify the highest-impact opportunities and biggest risks.", modules: ["orghealth", "scan", "heatmap", "readiness", "changeready", "clusters", "recommendations", "mgrcap", "skills"] },
  { id: "design", label: "Design", icon: "✏️", color: "var(--sage)", desc: "Architect the future state", guidance: "Design your future state — redesign roles, restructure the operating model, and plan your workforce.", modules: ["design", "opmodel", "build", "bbba", "headcount", "quickwins", "rolecompare"] },
  { id: "simulate", label: "Simulate", icon: "⚡", color: "var(--dusk)", desc: "Model the impact before you commit", guidance: "Model different futures before committing. Adjust assumptions, compare scenarios, and build the business case.", modules: ["simulate"] },
  { id: "mobilize", label: "Mobilize", icon: "🚀", color: "var(--amber)", desc: "Make it happen", guidance: "Build your transformation roadmap, engage stakeholders, and generate the deliverables.", modules: ["plan", "story", "archetypes", "mgrdev", "reskill", "marketplace", "export"] },
];

export const MODULES = [
  { id: "dashboard", icon: "🎯", title: "Transformation Dashboard", desc: "Executive summary across all phases", color: "var(--amber)", phase: "discover", views: ["org","custom"] },
  { id: "jobarch", icon: "🏗️", title: "Job Architecture", desc: "Enterprise job catalogue, hierarchy, career framework & validation", color: "var(--amber)", phase: "discover", views: ["org","job","employee","custom"], empTitle: "Career Path", jobTitle: "Role in Context", empDesc: "Your career trajectory and development", jobDesc: "Where this role sits in the hierarchy" },
  { id: "snapshot", icon: "📊", title: "Workforce Snapshot", desc: "See your people, structure, and readiness baseline", color: "var(--amber)", phase: "discover", views: ["org","job","employee","custom"], empTitle: "Employee Profile", jobTitle: "Job Profile", empDesc: "Your profile, team, and AI impact", jobDesc: "Role incumbents, comp, and AI scores" },
  { id: "orghealth", icon: "🏥", title: "Org Health Scorecard", desc: "Auto-calculated metrics with industry benchmarks", color: "var(--amber)", phase: "diagnose", views: ["org","custom"] },
  { id: "scan", icon: "🔬", title: "AI Opportunity Scan", desc: "Find where AI creates the most value", color: "var(--amber)", phase: "diagnose", views: ["org","job","employee","custom"], empTitle: "AI Impact on My Role", jobTitle: "AI Impact on This Job" },
  { id: "heatmap", icon: "🔥", title: "AI Impact Heatmap", desc: "Automation potential by function × job family", color: "var(--coral)", phase: "diagnose", views: ["org","custom"] },
  { id: "clusters", icon: "🔗", title: "Role Clustering", desc: "Group similar roles, identify consolidation candidates", color: "var(--dusk)", phase: "diagnose", views: ["org","custom"] },
  { id: "readiness", icon: "🎯", title: "AI Readiness", desc: "Individual and team readiness for AI transformation", color: "var(--sky-gold)", phase: "diagnose", views: ["org","job","employee","custom"], empTitle: "My Readiness", empDesc: "Your personal AI readiness scores" },
  { id: "mgrcap", icon: "👔", title: "Manager Capability", desc: "Assess manager readiness and identify champions", color: "var(--dusk)", phase: "diagnose", views: ["org","custom"] },
  { id: "recommendations", icon: "🤖", title: "AI Recommendations", desc: "AI-generated transformation recommendations ranked by impact", color: "var(--amber)", phase: "diagnose", views: ["org","job","custom"] },
  { id: "skills", icon: "🧠", title: "Skills & Talent", desc: "Inventory, gap analysis, and adjacency mapping", color: "var(--dusk)", phase: "diagnose", views: ["org","job","employee","custom"], empTitle: "My Skills", jobTitle: "Role Skills", empDesc: "Your skill profile and development gaps", jobDesc: "Skills required for this role" },
  { id: "skillsmap", icon: "🗺️", title: "Skills Map Engine", desc: "O*NET-powered skills library, job matching, and mapping", color: "var(--amber)", phase: "diagnose", views: ["org","job","custom"], jobTitle: "Role Skills Map", jobDesc: "O*NET skill profile for this role" },
  { id: "bbba", icon: "🔀", title: "Build/Buy/Borrow/Auto", desc: "Talent sourcing strategy per redesigned role", color: "var(--dusk)", phase: "design", views: ["org","custom"] },
  { id: "headcount", icon: "👥", title: "Headcount Planning", desc: "Current to future workforce waterfall", color: "var(--dusk)", phase: "design", views: ["org","custom"] },
  { id: "design", icon: "✏️", title: "Work Design Lab", desc: "Redesign tasks, roles, and time allocation job by job", color: "var(--sage)", phase: "design", views: ["org","job","custom"] },
  { id: "simulate", icon: "⚡", title: "Impact Simulator", desc: "Model scenarios, costs, and redeployment outcomes", color: "var(--dusk)", phase: "simulate", views: ["org","job","employee","custom"], empTitle: "How AI Affects Me", jobTitle: "Role Scenario", empDesc: "Personal impact of AI transformation", jobDesc: "Scenario modeling for this specific role" },
  { id: "build", icon: "🏗️", title: "Org Design Studio", desc: "Reshape spans, layers, and structure across the org", color: "var(--amber)", phase: "design", views: ["org","job","employee","custom"], empTitle: "My Org Chart", jobTitle: "Structural Context", empDesc: "Your reporting line and team structure", jobDesc: "Where this role sits structurally" },
  { id: "reorg", icon: "🔄", title: "Org Restructuring", desc: "Design future-state org structures with drag-and-drop canvas", color: "var(--amber)", phase: "design", views: ["org","job","custom"], jobTitle: "Role Restructuring", jobDesc: "How this role changes in the restructure" },
  { id: "reskill", icon: "📚", title: "Reskilling Pathways", desc: "Per-employee learning plans and timelines", color: "var(--dusk)", phase: "mobilize", views: ["org","employee","custom"], empTitle: "My Learning Path", empDesc: "Your personal reskilling journey" },
  { id: "marketplace", icon: "🏪", title: "Talent Marketplace", desc: "Match internal candidates to redesigned roles", color: "var(--amber)", phase: "mobilize", views: ["org","custom"] },
  { id: "skillshift", icon: "🔄", title: "Skill Shift Index", desc: "Net skill movement — declining, amplified, and net-new skills", color: "var(--dusk)", phase: "discover", views: ["org","custom"] },
  { id: "changeready", icon: "📈", title: "Change Readiness", desc: "4-quadrant segmentation and intervention mapping", color: "var(--coral)", phase: "diagnose", views: ["org","custom"] },
  { id: "archetypes", icon: "🎭", title: "Readiness Archetypes", desc: "Consultant-grade workforce archetypes with engagement playbooks", color: "var(--sky-gold)", phase: "mobilize", views: ["org","custom"] },
  { id: "story", icon: "📖", title: "Transformation Story", desc: "AI-generated executive narrative for board presentations", color: "var(--amber)", phase: "mobilize", views: ["org","custom"] },
  { id: "mgrdev", icon: "🎓", title: "Manager Development", desc: "Targeted development plans for people managers", color: "var(--dusk)", phase: "mobilize", views: ["org","custom"] },
  { id: "plan", icon: "🚀", title: "Change Planner", desc: "Sequence initiatives and manage transformation risk", color: "var(--coral)", phase: "mobilize", views: ["org","job","employee","custom"], empTitle: "My Change Journey", jobTitle: "Role Change Plan", empDesc: "Your personal transformation timeline", jobDesc: "Change initiatives affecting this role" },
  { id: "export", icon: "📋", title: "Export & Report", desc: "Generate your board-ready transformation report", color: "var(--coral)", phase: "mobilize", views: ["org","job","employee","custom"] },
  { id: "opmodel", icon: "🧬", title: "Operating Model Lab", desc: "Explore architecture patterns across functions", color: "var(--amber)", phase: "design", views: ["org","custom"] },
  // om_canvas is accessed from within OperatingModelLab, not as a standalone module
  { id: "rolecompare", icon: "⚖️", title: "Role Comparison", desc: "Side-by-side current vs. redesigned role analysis", color: "var(--sky-gold)", phase: "design", views: ["org","job","custom"] },
  { id: "quickwins", icon: "⚡", title: "Quick-Win Identifier", desc: "Find highest ROI, lowest effort AI opportunities", color: "var(--sage)", phase: "design", views: ["org","custom"] },
];
...
export const SIM_PRESETS: Record<string, { label: string; adoption: number; timeline: number; ramp: number; color: string }> = {
  conservative: { label: "Conservative", adoption: 0.3, timeline: 18, ramp: 0.6, color: "var(--amber)" },
  balanced: { label: "Balanced", adoption: 0.6, timeline: 12, ramp: 0.8, color: "var(--sage)" },
  aggressive: { label: "Aggressive", adoption: 0.8, timeline: 12, ramp: 0.9, color: "var(--dusk)" },
};
...
export const SKILLS_TAXONOMY: { domain: string; color: string; icon: string; skills: { name: string; profLevels: string[]; industries?: string[] }[] }[] = [
  { domain: "Technical & Engineering", color: "var(--amber)", icon: "⚙️", skills: [
  ...
  { domain: "Leadership & Management", color: "var(--dusk)", icon: "👔", skills: [
  ...
  { domain: "Analytical & Quantitative", color: "var(--sage)", icon: "📊", skills: [
  ...
  { domain: "Communication & Influence", color: "var(--amber)", icon: "💬", skills: [
  ...
  { domain: "Digital & AI Fluency", color: "var(--sky-gold)", icon: "🤖", skills: [
  ...
  { domain: "Domain-Specific", color: "var(--coral)", icon: "🏢", skills: [
