...
export function OrgDesignStudio({ onBack, model, f, odsState, setOdsState, viewCtx, jobStates }: { onBack: () => void; model: string; f: Filters; odsState: { activeScenario: number; view: string }; setOdsState: (s: { activeScenario: number; view: string }) => void; viewCtx?: ViewContext; jobStates?: Record<string, JobDesignState> }) {
  ...
  const DChip = ({ a, b, inv }: { a: number; b: number; inv?: boolean }) => {
    ...
    return <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[15px] font-bold" style={{ color: c, background: pos ? "rgba(139,168,122,0.1)" : neg ? "rgba(232,122,93,0.1)" : "transparent" }}>{ar}{fmt(Math.abs(diff))}</span>;
  };
  ...
  return <div style={{ margin: '-24px -24px 0', padding: '24px 24px 0', minHeight: '100vh' }}>
    ...
    {hasRealData ? <div className="bg-[rgba(139,168,122,0.08)] border border-[var(--success)]/30 rounded-lg px-4 py-2 mb-4 text-[15px] text-[var(--success)]">✓ Using your uploaded workforce data to model departments</div> : <div className="bg-[rgba(244,168,58,0.08)] border border-[var(--warning)]/30 rounded-lg px-4 py-2 mb-4 text-[15px] text-[var(--warning)]" style={{ display: 'flex', alignItems: 'center', gap: 10 }}><span style={{ background: 'rgba(244,168,58,0.15)', borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' as const }}>SAMPLE DATA</span><span>Upload your workforce data for real analysis — current charts use generated sample data</span></div>}
      ...
      return <>
        ...
        <div ref={drawerRef} onKeyDown={handleDrawerKeyDown} role="dialog" aria-label="Scenario Builder" className="fixed top-0 right-0 h-full w-[400px] z-50 bg-[#0f172a] border-l border-[var(--border)] shadow-2xl overflow-y-auto transition-transform duration-300" style={{ boxShadow: "0 0 60px rgba(0,0,0,0.5)" }}>
          <div className="p-6">
            ...
            {/* UX Guidance */}
            <div className="rounded-lg bg-[rgba(244,168,58,0.06)] border border-[rgba(244,168,58,0.15)] p-3 mb-5 text-[12px] text-[var(--text-secondary)] leading-relaxed">
