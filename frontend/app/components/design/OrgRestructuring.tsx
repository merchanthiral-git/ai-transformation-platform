...
export function OrgRestructuring({ model, f, onBack, onNavigate, viewCtx, jobStates }: {
    ...
    const renderSvgNode = (node: any, idx: number) => {
      ...
      return (
        ...
        >
          ...
          <text x={14} y={20} fontSize={13} fontWeight={500} fill="#1C2B3A"
          ...
          {node.status !== "current" && (
            <>
              <rect x={w - 60} y={4} width={52} height={14} rx={3}
                fill={node.status === "added" ? "rgba(16,185,129,0.12)"
                  : node.status === "eliminated" ? "rgba(239,68,68,0.12)"
          ...
          {hasChildren && (
            <g transform={`translate(${w / 2 - 14}, ${h - 4})`}
              ...
              <rect x={0} y={0} width={28} height={16} rx={8} fill="#1C2B3A" />
          ...
          >
            ...
            <div style={{
              ...
              <button
                ...
                style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 24, height: 24, background: "none", border: "0.5px solid rgba(28,43,58,0.15)", borderRadius: 4, cursor: "pointer", color: "#1C2B3A" }}
              ><Plus size={12} /></button>
              <button
              ...
              <button
                ...
                style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 3, padding: "2px 8px", height: 24, background: "none", border: "0.5px solid rgba(28,43,58,0.15)", borderRadius: 4, cursor: "pointer", color: "#1C2B3A", fontSize: 11, fontWeight: 500 }}
          ...
          {selectedNode && selectedNode.status !== "eliminated" && (
            <div style={{
              ...
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 12px", fontSize: 12, fontWeight: 500, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 6, cursor: "pointer", color: "var(--risk)" }}
              ...
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 12px", fontSize: 12, fontWeight: 500, background: inlineNoteOpen ? "rgba(59,130,246,0.08)" : "var(--surface-2)", border: `1px solid ${inlineNoteOpen ? "rgba(59,130,246,0.3)" : "var(--border)"}`, borderRadius: 6, cursor: "pointer", color: inlineNoteOpen ? "#3B82F6" : "var(--text-primary)" }}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {sorted.map(s => (
                <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: 10, border: "1px solid var(--border)", background: "var(--surface-2)", cursor: "pointer", transition: "border-color 0.15s" }}
                  ...
                  <div style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(212,134,10,0.12)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    ...
                    {scenarioMenuOpen === s.id && (
                      <div style={{ position: "absolute", top: 32, right: 0, zIndex: 20, background: "#fff", border: "1px solid var(--border)", borderRadius: 8, boxShadow: "0 4px 16px rgba(0,0,0,0.12)", minWidth: 140, overflow: "hidden" }} onClick={e => e.stopPropagation()}>
                        ...
                        <button onClick={() => handleDeleteScenario(s.id)} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "8px 14px", fontSize: 13, background: "none", border: "none", cursor: "pointer", color: "var(--risk)", textAlign: "left" }}
                          onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(239,68,68,0.06)"; }} onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = "none"; }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {savedScenarios.map(s => {
                  ...
                  return (<button key={s.id} onClick={() => setCompareSelection(prev => { if (prev.includes(s.id)) return prev.filter(id => id !== s.id); if (prev.length >= 4) { showToast("Maximum 4 scenarios"); return prev; } return [...prev, s.id]; })}
                    style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 14px", borderRadius: 8, fontSize: 13, border: isSel ? "2px solid var(--accent-primary)" : "1px solid var(--border)", background: isSel ? "rgba(212,134,10,0.08)" : "#fff", color: isSel ? "var(--accent-primary)" : "var(--text-primary)", fontWeight: isSel ? 600 : 400, cursor: "pointer" }}>
                  ...
                  <tbody>{mDefs.map(def => {
                    ...
                    return (<tr key={def.key} style={{ borderBottom: "1px solid var(--border)" }}>
                      ...
                      {cItems.map((it, i) => {
                        ...
                        return <td key={i} style={{ textAlign: "right", padding: "10px 12px", fontFamily: "var(--ff-mono)", fontWeight: 600, borderLeft: "1px solid var(--border)", color: isB ? "var(--text-muted)" : isBest ? "var(--success)" : isWorst ? "var(--risk)" : "var(--text-primary)", background: isBest ? "rgba(16,185,129,0.06)" : isWorst ? "rgba(239,68,68,0.04)" : "transparent" }}>{fm}</td>;
                      })}
                    </tr>);
                  })}</tbody>
                </table>
  ...
  const renderEntryPoint = () => {
    ...
    const cards: { icon: React.ReactNode; title: string; desc: string; action: () => void }[] = [
      {
        icon: <Eye size={24} style={{ color: "var(--navy, #1C2B3A)" }} />,
      ...
      {
        icon: <GitBranch size={24} style={{ color: "var(--navy, #1C2B3A)" }} />,
    ...
    return (
      <div style={{ padding: 32 }}>
        ...
        <div style={{ maxWidth: 1160, margin: "0 auto" }}>
          <h2 style={{
            fontSize: 20, fontWeight: 600, color: "var(--navy, #1C2B3A)",
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 16, marginBottom: 40 }}>
            {cards.map((c) => (
              ...
              >
                {c.icon}
                <div style={{ fontSize: 15, fontWeight: 500, color: "var(--navy, #1C2B3A)" }}>
                  {c.title}
                </div>
          ...
          <div style={{ marginBottom: 32 }}>
            <h3 style={{
              fontSize: 14, fontWeight: 600, color: "var(--navy, #1C2B3A)",
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                {recentScenarios.map((s, i) => (
                  ...
                  >
                    ...
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 500, color: "var(--navy, #1C2B3A)" }}>
          ...
          <div style={{
            ...
            <span
              ...
              style={{ color: "var(--blue-primary, #3B82F6)", cursor: "pointer", fontWeight: 500 }}
            >
    ...
    <div style={{ padding: 32 }}>
      ...
      <div style={{ marginBottom: 8 }}>
        <span
          ...
          style={{
            fontSize: 13, color: "var(--blue-primary, #3B82F6)", cursor: "pointer",
