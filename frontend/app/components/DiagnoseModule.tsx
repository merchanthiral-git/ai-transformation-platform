...
export function AiOpportunityScan({ model, f, onBack, onNavigate, viewCtx }: { model: string; f: Filters; onBack: () => void; onNavigate?: (id: string) => void; viewCtx?: ViewContext }) {
  ...
  const funcHeadcounts = useMemo(() => {
    ...
    const fd = (overviewData as Record<string, unknown>)?.func_distribution;
  ...
  return <div>
    ...
    {sub === "ai" && (() => { const s = (data?.summary ?? { tasks_scored: 0, quick_wins: 0, total_time_impact: 0, avg_risk: 0 }) as AIPrioritySummary; const top10 = (data?.top10 ?? []) as Record<string, unknown>[]; const quickWins = top10.filter(t => String(t["AI Impact"] || t.ai_impact || "").toLowerCase() === "high" && (Number(t["Current Time Spent %"] || t.time_pct || 0) >= 10) && String(t["Logic"] || t.logic || "").toLowerCase() === "deterministic");
      ...
      const funcMap = new Map<string, { tasks: number; autoHrs: number; impactSum: number; quickWins: number; scoredRoles: Set<string> }>();
      for (const t of top10) {
        const func = String(t["Function ID"] || t.function || t["Function"] || "Unknown");
        if (!func || func === "Unknown") continue;
        const entry = funcMap.get(func) || { tasks: 0, autoHrs: 0, impactSum: 0, quickWins: 0, scoredRoles: new Set() };
        entry.tasks++;
        const timePct = Number(t["Current Time Spent %"] || t.time_pct || 0);
        const aiScore = Number(t["AI Score"] || t.ai_score || t["Time Saved %"] || 0);
        entry.autoHrs += (timePct / 100) * (aiScore / 100) * 40;
        ...
        if (role) entry.scoredRoles.add(role);
        funcMap.set(func, entry);
      }
      // Include functions from overview even if no scored tasks
      for (const fname of Object.keys(funcHeadcounts)) {
        if (!funcMap.has(fname)) funcMap.set(fname, { tasks: 0, autoHrs: 0, impactSum: 0, quickWins: 0, scoredRoles: new Set() });
      }
      const allFuncRows = Array.from(funcMap.entries()).map(([name, d]) => {
        const totalHC = funcHeadcounts[name] || d.scoredRoles.size || 0;
        const scoredRoleCount = d.scoredRoles.size;
        const avgImpact = d.tasks > 0 ? Math.round(d.impactSum / d.tasks) : 0;
        const hcComponent = Math.round(totalHC * avgImpact * 0.004);
        const hrsComponent = Math.round(d.autoHrs * 4);
        const qwComponent = Math.round(d.quickWins * 10 * 0.2);
        const rawComposite = hcComponent + hrsComponent + qwComponent;
        ...
        const included = !excludedFuncs[name];
        return { name, headcount: totalHC, scoredRoles: scoredRoleCount, tasks: d.tasks, autoHrs: Math.round(d.autoHrs * 10) / 10, avgImpact, quickWins: d.quickWins, composite, rawComposite, hcComponent, hrsComponent, qwComponent, leadershipReadiness: lr, penalized: lr === "Resistant", included };
      }).sort((a, b) => {
        if (a.included !== b.included) return a.included ? -1 : 1;
        return b.composite - a.composite;
      });
      const includedRows = allFuncRows.filter(r => r.included);
      let includedRank = 0;
      ...
      {allFuncRows.length > 0 && <div className="mb-5">
        <div className="flex items-baseline justify-between mb-3">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[11px] text-[var(--text-muted)]">Showing {allFuncRows.length} functions · {includedRows.length} included</span>
            {Object.values(excludedFuncs).some(Boolean) && <button onClick={() => setExcludedFuncs({})} className="text-[11px] text-[var(--accent-primary)] font-semibold">Reset exclusions</button>}
          </div>
        </div>
        <div className="overflow-auto rounded-xl border border-[var(--border)]" style={{ maxHeight: 500 }}>
          <table className="w-full text-[13px]" style={{ borderCollapse: "collapse", fontVariantNumeric: "tabular-nums" }}>
            <thead><tr className="bg-[var(--surface-2)] sticky top-0 z-10">
              <th className="px-2 py-2 text-center text-[var(--text-muted)] font-semibold border-b border-[var(--border)] w-10"></th>
              <th className="px-2 py-2 text-center text-[var(--text-muted)] font-semibold border-b border-[var(--border)] w-16">Rank</th>
              <th className="px-3 py-2 text-left text-[var(--text-muted)] font-semibold border-b border-[var(--border)]">Function</th>
              <th className="px-3 py-2 text-center text-[var(--text-muted)] font-semibold border-b border-[var(--border)]">Headcount</th>
            ...
            <tbody>{allFuncRows.map((r) => {
              const rank = r.included ? ++includedRank : null;
              const isTop5 = rank !== null && rank <= 5;
              return <tr key={r.name} className="border-b border-[var(--border)] hover:bg-[rgba(59,130,246,0.06)] transition-colors" style={{ borderLeft: isTop5 ? "3px solid #3B82F6" : "3px solid transparent", opacity: r.included ? 1 : 0.4 }}>
                <td className="px-2 py-2 text-center"><input type="checkbox" checked={r.included} onChange={() => setExcludedFuncs(prev => ({ ...prev, [r.name]: r.included }))} style={{ accentColor: "var(--accent-primary)", cursor: "pointer" }} /></td>
                <td className="px-2 py-2 text-center font-semibold" style={{ color: isTop5 ? "#3B82F6" : "var(--text-muted)" }}>{rank ?? "—"}{isTop5 && <span className="ml-1" style={{ color: "#3B82F6", fontSize: 10 }}>●</span>}</td>
                <td className="px-3 py-2"><div className="font-semibold text-[var(--text-primary)]">{r.name}</div><div className="text-[10px] text-[var(--text-muted)]">{r.scoredRoles} of {r.headcount} roles scored</div></td>
                ...
                <td className="px-3 py-2 text-center text-[var(--text-secondary)]">{r.autoHrs}</td>
                <td className="px-3 py-2 text-center"><div className="relative"><span className="text-[var(--text-secondary)]">{r.avgImpact}</span><div className="absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] rounded" style={{ width: `${r.avgImpact}%`, maxWidth: 40, background: "rgba(59,130,246,0.35)" }} /></div></td>
                <td className="px-3 py-2 text-center text-[var(--text-secondary)]" style={{ fontWeight: 500 }}>{r.quickWins}</td>
                ...
                <td className="px-3 py-2 text-center" title={`HC×Impact: ${r.hcComponent} (40%) + Hours: ${r.hrsComponent} (40%) + QW: ${r.qwComponent} (20%)${r.penalized ? ` × 0.5 (Resistant)` : ""} = ${r.composite}`}>
                  <span className="font-semibold" style={{ color: isTop5 ? "#3B82F6" : "var(--text-primary)" }}>{r.composite}</span>
                  {r.penalized && <span style={{ marginLeft: 3, fontSize: 10, color: "#EF4444" }}>↓</span>}
      ...
      {allFuncRows.length === 0 && top10.length === 0 && <div className="mb-5 p-4 rounded-xl border border-[var(--border)] text-center text-[13px] text-[var(--text-muted)]">Score tasks in the <button onClick={() => setSub("heatmap")} className="text-[var(--accent-primary)] font-semibold">Impact Heatmap tab</button> to populate function rankings.</div>}
