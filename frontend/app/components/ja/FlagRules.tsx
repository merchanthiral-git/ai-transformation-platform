...
const SEV_COLORS: Record<string, string> = { error: "#EF4444", warning: "#F97316", info: "#3B82F6" };
...
const S = {
  ...
  tab: (active: boolean) => ({ padding: "8px 16px", fontSize: "var(--text-xs)", fontWeight: active ? "var(--fw-semi)" : "var(--fw-medium)", color: active ? "#3B82F6" : "var(--text-muted)", borderBottom: active ? "2px solid #3B82F6" : "2px solid transparent", background: "none", border: "none", cursor: "pointer" }) as React.CSSProperties,
  ...
  groupHeader: (sev: string) => ({
    ...
    background: sev === "error" ? "rgba(239,68,68,0.06)" : sev === "warning" ? "rgba(249,115,22,0.06)" : "rgba(59,130,246,0.04)",
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.summaryBar}>
        <div style={S.summaryItem("#EF4444")}>
          <span style={{ fontSize: "var(--text-base)", fontWeight: "var(--fw-bold)", color: "#EF4444" }}>{errorCount}</span>
          <span style={{ color: "var(--text-muted)" }}>Errors</span>
        </div>
        <div style={S.summaryItem("#F97316")}>
          <span style={{ fontSize: "var(--text-base)", fontWeight: "var(--fw-bold)", color: "#F97316" }}>{warnCount}</span>
          <span style={{ color: "var(--text-muted)" }}>Warnings</span>
        </div>
        <div style={S.summaryItem("#3B82F6")}>
          <span style={{ fontSize: "var(--text-base)", fontWeight: "var(--fw-bold)", color: "#3B82F6" }}>{infoCount}</span>
              ...
              <div key={group.rule.id} style={S.section}>
                ...
                {expanded.has(group.rule.id) && group.violations.map(v => (
                  <div key={v.id} style={S.violationRow}>
                    ...
                    <button style={{ ...S.suppressBtn, color: "#3B82F6" }}
                    ...
                    {suppressingId === v.id ? (
                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        ...
                        <button style={{ ...S.suppressBtn, color: "#22C55E" }} onClick={() => suppressViolation(v.id)} disabled={!suppressNote.trim()}>
                          <Check size={10} />
                        </button>
