...
const S = {
  ...
  issueRow: (sev: string) => ({
    ...
    borderLeft: `3px solid ${sev === "critical" ? "var(--coral)" : sev === "warning" ? "var(--amber)" : "var(--amber)"}`,
  ...
  arrow: { fontSize: "var(--text-sm)", color: "var(--amber)", fontWeight: "var(--fw-semi)" } as React.CSSProperties,
  ...
  bar: (pct: number) => ({ width: `${Math.max(pct, 2)}%`, height: 18, background: "var(--amber)", borderRadius: 3, transition: "width 0.3s" }) as React.CSSProperties,
};
...
export default function CatalogueHealth({ model, projectId }: Props) {
  ...
  const scoreColor = healthScore >= 70 ? "var(--sage)" : healthScore >= 40 ? "var(--amber)" : "var(--coral)";
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.grid}>
        ...
        <div style={S.kpi(d.title_inflation_rate > 30 ? "var(--coral)" : "var(--sage)")}>
        ...
        <div style={S.kpi(d.ghost_roles_count > 0 ? "var(--amber)" : "var(--sage)")}>
        ...
        <div style={S.kpi(d.duplicate_titles_count > 5 ? "var(--amber)" : "var(--sage)")}>
          <div style={S.kpiLabel}>Duplicate Titles</div>
          <div style={S.kpiValue}>{d.duplicate_titles_count}</div>
        </div>
        <div style={S.kpi(d.roles_missing_family > 0 ? "var(--amber)" : "var(--sage)")}>
      ...
      {issues.length > 0 && (
        <div style={S.section}>
          <div style={S.sectionTitle}><AlertTriangle size={15} style={{ color: "var(--amber)" }} /> Issues Found</div>
          {issues.map((issue, i) => (
            <div key={i} style={S.issueRow(issue.severity)}>
              <div style={{ flexShrink: 0, marginTop: 1 }}>
                {issue.severity === "critical" ? <AlertTriangle size={14} style={{ color: "var(--coral)" }} /> : <FileWarning size={14} style={{ color: "var(--amber)" }} />}
              </div>
              <div>
      ...
      <div style={S.section}>
        <div style={{ ...S.sectionTitle, justifyContent: "space-between" }}>
          <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {clusters.length > 0 && <span style={{ fontSize: 11, color: "var(--amber)", fontWeight: "var(--fw-medium)" }}>{clusters.length} clusters found</span>}
          </span>
          <button style={{ background: "none", border: "none", fontSize: "var(--text-xs)", color: "var(--amber)", cursor: "pointer", fontWeight: "var(--fw-medium)" }}
