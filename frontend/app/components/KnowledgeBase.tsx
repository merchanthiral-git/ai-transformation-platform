...
export const KNOWLEDGE_BASE: Record<string, KBEntry> = {
  ...
  archetypes: {
    ...
    category: "Mobilize",
    categoryColor: CAT_COLORS["Mobilize"],
    summary: "Segments the workforce into behavioral profiles — Early Adopters, Pragmatic Majority, Skeptics, and Support-First Adopters — with tailored engagement strategies for each group.",
    ...
    what: [
      { heading: "The four archetypes", body: "Early Adopters (15-20% of workforce): high readiness, embrace change, actively seek new tools. Pragmatic Majority (40-50%): willing but cautious, need evidence and peer validation. Skeptics (20-25%): resistant but persuadable with data, transparency, and demonstrated wins. Support-First Adopters (5-15%): need intensive one-on-one support and often have legitimate concerns worth hearing." },
    ...
    why: [{ heading: "Why it matters", body: "Quantitative readiness scores tell you who is ready; archetypes tell you why they are or aren't. A Skeptic with a readiness score of 2.5 needs a fundamentally different approach than a Support-First Adopter with the same score. The archetype determines the intervention, not just the score." }],
    ...
    bestPractices: ["Don't treat Support-First Adopters as problems — their concerns often reveal real issues that need addressing", "Deploy Champions at a 1:5 ratio (one champion per five people in their team)"],
    pitfalls: ["Assuming archetype distribution is static — people move between archetypes as the transformation progresses", "Investing all energy in Support-First Adopters while ignoring the Pragmatic Majority who represent the largest population"],
    related: ["changeready", "plan", "mgrdev", "reskill"],
    scenario: "A retail organization discovered their archetype distribution was 12% Early Adopters, 48% Pragmatics, 28% Skeptics, and 12% Support-First Adopters. By deploying 60 Champions (Early Adopters) across 300 teams, running 3 visible pilot programs (Proof Points for Pragmatics), holding transparent Q&A sessions (for Skeptics), and offering 1:1 coaching for Support-First Adopters, they achieved 78% adoption within 8 months — compared to a 45% industry average for similar-scale transformations.",
  },
...
function SlideViewer({ slides, accentColor }: { slides: Slide[]; accentColor: string }) {
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => { if (e.key === "ArrowRight") go(idx + 1); if (e.key === "ArrowLeft") go(idx - 1); };
...
function buildSlides(entry: KBEntry, sectionId: string): Slide[] {
  ...
  if (sectionId === "practices") {
    ...
    entry.bestPractices.forEach((p, i) => {
      ...
      slides.push({
        ...
        content: <div style={{ borderLeft: "4px solid var(--success)", background: "rgba(139,168,122,0.06)", borderRadius: "0 12px 12px 0", padding: "16px 20px" }}>
    ...
    entry.pitfalls.forEach((p, i) => {
      ...
      slides.push({
        ...
        content: <div style={{ borderLeft: "4px solid var(--risk)", background: "rgba(232,122,93,0.06)", borderRadius: "0 12px 12px 0", padding: "16px 20px" }}>
  ...
  if (sectionId === "scenario") {
    ...
    const slides: Slide[] = [
      ...
      { title: "The Situation", content: <div style={{ background: "var(--surface-2)", borderRadius: 14, padding: 20, border: "1px solid rgba(244,168,58,0.12)", fontSize: 16, lineHeight: 1.8 }}>{sentences.slice(0, third).join(" ")}</div> },
    ...
    if (third * 2 < sentences.length) slides.push({ title: "Results & Impact", content: <div style={{ background: "var(--surface-2)", borderRadius: 14, padding: 20, border: "1px solid rgba(139,168,122,0.12)", fontSize: 16, lineHeight: 1.8 }}>{sentences.slice(third * 2).join(" ")}</div> });
    return slides;
  }
  ...
  if (presenting) {
    return <div style={{ position: "fixed", inset: 0, zIndex: 999999, background: "var(--surface-1)", display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "16px 24px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <button onClick={() => setPresenting(false)} style={{ padding: "6px 14px", borderRadius: 8, fontSize: 15, fontWeight: 600, background: "rgba(244,168,58,0.1)", border: "1px solid rgba(244,168,58,0.2)", color: "var(--accent-primary)", cursor: "pointer" }}>Exit Presentation</button>
      </div>
    ...
    <div className="w-full rounded-2xl border border-[var(--border)] bg-[var(--surface-1)] shadow-2xl flex animate-card-enter" style={{ maxHeight: "90vh", minWidth: 800, maxWidth: 1100 }} onClick={e => e.stopPropagation()}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="sticky top-0 z-10 bg-[var(--surface-1)] border-b border-[var(--border)] px-6 py-4 rounded-tr-2xl">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setPresenting(true)} title="Present fullscreen" style={{ padding: "4px 10px", borderRadius: 6, fontSize: 15, fontWeight: 600, background: "rgba(244,168,58,0.06)", border: "1px solid rgba(244,168,58,0.12)", color: "rgba(244,168,58,0.6)", cursor: "pointer" }} onMouseEnter={e => { e.currentTarget.style.color = "var(--accent-primary)"; e.currentTarget.style.borderColor = "rgba(244,168,58,0.3)"; }} onMouseLeave={e => { e.currentTarget.style.color = "rgba(244,168,58,0.6)"; e.currentTarget.style.borderColor = "rgba(244,168,58,0.12)"; }}>🖥 Present</button>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <SlideViewer key={activeSection} slides={slides} accentColor={accentColor} />
